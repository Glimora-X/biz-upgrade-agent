...
5. **升级脚本** - 在集成终端中执行升级脚本，保持彩色输出，**支持失败后重试**
6. **特性分支单测** - 可选在特性分支运行单元测试，**支持失败后重试**
7. **代码提交** - 自动提交升级变更到本地特性分支
8. **分支合并** - 合并特性分支到目标分支并推送
9. **目标分支单测** - 可选在目标分支运行单元测试验证，**支持失败后重试**
...
- **Test 环境**：`源码分支(plus-upgrade-test) → 本地特性分支 → 目标分支（test-260127 ）`
